...
| 历史基线批次 | **9 / 10** | 旧版单次评测一次失败来自网络 TLS 瞬断；不冒充当前 50 任务结果 |
...
- Node.js 24+
- npm 10+
- 可选：DeepSeek API Key；OpenAI API Key 仅用于自行恢复仓库保留的早期 Realtime 实验，不属于当前产品入口
...
1. 在当前修复提交上完整复跑 50 条语义任务 × 3 次，与首轮 113/150 基线对比，并人工复核失败分类、P95 延迟与 Token；完成前不宣称“修复后全量通过率”。
