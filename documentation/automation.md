...
- Steering：当前产品由 DeepSeek system prompt 决定模型应如何规划、何时澄清以及回复风格；早期 Realtime instructions 仅随探索代码保留，没有独立产品入口。
...
- Agent API 的核心返回包括 `message/sessionId/vehicle/traces/model/turns/totalTokens/plan/promptVersion/toolVersion/stateVersion/occupantRole/operation/requestedModel/resolvedModel/executionSource`；未调用供应商模型的确定性路径将 `resolvedModel` 置为 `null`。
