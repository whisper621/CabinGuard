...
export default function AgentLab() {
  ...
  return <main className="min-h-screen bg-[#070b14] text-slate-100">
    <header className="sticky top-0 z-[1001] border-b border-white/10 bg-[#080d18]/95 backdrop-blur-xl"><div className="mx-auto flex max-w-[1720px] flex-wrap items-center justify-between gap-4 px-5 py-4 xl:px-8"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 to-cyan-400 font-black text-slate-950">CG</span><div><div className="flex flex-wrap items-center gap-2"><h1 className="text-lg font-semibold tracking-tight">CabinGuard 座舱系统</h1><span className="rounded-full border border-cyan-400/25 bg-cyan-400/10 px-2.5 py-1 text-xs font-medium text-cyan-300">1 个主 Agent + 1 个导航领域 Agent · 14 工具 · 25 信号</span><span className={`rounded-full border px-2.5 py-1 text-xs font-medium ${usesExternalCabinApi ? "border-emerald-400/25 bg-emerald-400/10 text-emerald-300" : "border-amber-400/25 bg-amber-400/10 text-amber-300"}`}>{usesExternalCabinApi ? "Python 核心已连接 · 真实地图" : "请启动 Python 服务以开放真实算路"}</span></div><p className="mt-0.5 text-xs text-slate-500">驾驶任务规划 / 真实地点检索 / 策略执行 / 可复核轨迹</p></div></div><div className="flex flex-wrap items-center gap-2 text-sm"><Link className="rounded-lg px-3 py-2 text-cyan-300 hover:bg-cyan-400/10 hover:text-white" href="/mission">任务驾驶舱</Link><Link className="rounded-lg px-3 py-2 text-violet-300 hover:bg-violet-400/10 hover:text-white" href="/twin-lab">数字孪生</Link><Link className="rounded-lg px-3 py-2 text-violet-300 hover:bg-violet-400/10 hover:text-white" href="/ops">证据中心</Link><Link className="rounded-lg px-3 py-2 text-slate-400 hover:bg-white/5 hover:text-white" href="/system-lab">系统架构</Link><Link className="rounded-lg px-3 py-2 text-slate-400 hover:bg-white/5 hover:text-white" href="/evaluation">可靠性评测</Link><button className="rounded-lg border border-white/15 bg-white/5 px-3 py-2 text-slate-200 hover:bg-white/10" onClick={() => void reset()}>重置会话</button></div></div></header>
