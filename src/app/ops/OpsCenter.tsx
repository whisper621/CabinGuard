...
export default function OpsCenter() {
  ...
  return <div className="bg-[#050b16] text-slate-100"><div className="mx-auto max-w-[1680px] space-y-5 px-5 py-6 lg:px-8">
    <section className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]"><div className="rounded-[30px] border border-white/10 bg-[linear-gradient(120deg,#091728,#131329)] p-6"><p className="text-xs tracking-[0.2em] text-violet-300">ORCHESTRATION & EVIDENCE / 编排与证据</p><h1 className="mt-3 text-3xl font-semibold md:text-5xl">每一次“已执行”，<br />都能回答<span className="text-violet-300">为什么、谁允许、状态如何变化</span></h1><p className="mt-4 max-w-4xl text-sm leading-7 text-slate-400">证据账本由服务端 SQLite 追加写入，界面不参与安全裁决。面试演示时可从用户原话一路追溯到任务节点、ABAC 策略、工具回执与状态版本。</p></div><aside className="rounded-[30px] border border-white/10 bg-[#091323] p-5"><p className="text-xs text-slate-500">当前演示会话</p><div className="mt-3 flex gap-2"><input value={sessionId} onChange={(event) => setSessionId(event.target.value)} className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/20 px-3 py-2 font-mono text-xs outline-none focus:border-violet-400/40" placeholder="请先在智能座舱创建会话" /><button onClick={() => void loadEvidence(sessionId)} className="rounded-xl bg-violet-500 px-4 py-2 text-xs font-semibold text-white">{loading ? "刷新中" : "刷新证据"}</button></div>{!sessionId && <Link href="/" className="mt-4 block rounded-xl border border-cyan-400/20 bg-cyan-400/[0.05] px-4 py-3 text-center text-sm text-cyan-300">前往智能座舱创建会话 →</Link>}<p className="mt-4 text-xs leading-6 text-slate-500">安全边界：演示会话 ID 不是生产身份认证；真实量产系统还需接入车机账户、设备证书与硬件安全模块。</p></aside></section>
