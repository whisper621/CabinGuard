  ...
  return <main className="min-h-screen bg-[#070b14] text-slate-100">
    <header className="sticky top-0 z-20 border-b border-white/10 bg-[#080d18]/95 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1720px] flex-wrap items-center justify-between gap-4 px-5 py-4 xl:px-8">
        ...
        <nav className="flex flex-wrap items-center gap-2 text-sm"><Link href="/agent-lab" className="rounded-lg px-3 py-2 text-slate-400 hover:bg-white/5 hover:text-white">Agent Lab</Link><Link href="/evaluation" className="rounded-lg px-3 py-2 text-slate-400 hover:bg-white/5 hover:text-white">可靠性评测</Link><Link href="/case-study" className="rounded-lg px-3 py-2 text-slate-400 hover:bg-white/5 hover:text-white">产品案例</Link><span className={`rounded-lg border px-3 py-2 text-xs ${manifest ? "border-emerald-400/25 bg-emerald-400/10 text-emerald-300" : "border-amber-400/25 bg-amber-400/10 text-amber-300"}`}>{manifest ? `Python Registry · ${manifest.version}` : "正在连接 Python"}</span></nav>
      </div>
    </header>
    ...
    <div className="mx-auto max-w-[1720px] space-y-5 px-5 py-5 xl:px-8">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        <Metric value={manifest?.agentCount ?? "—"} label="Orchestrator Agent" note="单 Agent、多工具规划" />
